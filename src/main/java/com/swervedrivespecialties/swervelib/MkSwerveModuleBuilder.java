...
public class MkSwerveModuleBuilder {
   ...
   * or {@link MkModuleConfiguration#getDefaultSteerNEO()}, but you can use any custom module values
  ...
  /**
   * (Optional) Specify a shuffleboard container in which to print debug values for the swerve
   * module.
   *
   ...
   *
   * <p>Presets for SDS modules can be found in {@link SdsModuleConfigurations}.
   *
  ...
  /**
   * Specify details about the drive motor. The drive motor must be on the roboRIO canbus.
   *
   * @param motorType the {@link MotorType} of the motor
   * @param motorPort the CAN ID of the motor
   * @return the builder
    ...
    switch (motorType) {
      case FALCON:
        if (this.useDefaultSteerConfiguration)
          this.steerFactory =
              getFalcon500SteerFactory(MkModuleConfiguration.getDefaultSteerFalcon500());
  ...
  /**
   * Specify details about the steer motor. The steer motor must be on the roboRIO canbus.
   *
   * @param motorType the {@link MotorType} of the motor
   * @param motorPort the CAN ID of the motor
   * @return the builder
  ...
  /**
   * Specify details about the module's absolute encoder. The encoder must be on the roboRIO canbus.
   *
  ...
  public MkSwerveModuleBuilder withModuleId(int id) {
    this.moduleId = id;
    return this;
  }
  ...
  public SwerveModule build() {
    ...
    if (container == null) {
      return factory.create(driveMotorPort, driveCanbus, steerConfig, steerCanbus, moduleId);
    } else {
      return factory.create(
          container, driveMotorPort, driveCanbus, steerConfig, steerCanbus, moduleId);
    }
  }
}
