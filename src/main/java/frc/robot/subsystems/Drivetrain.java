...
import frc.robot.util.util6328.TunableNumber;
...
public class Drivetrain extends SubsystemBase {
  private final Pigeon2 gyroIO;
  private final TunableNumber autoDriveKp =
      new TunableNumber("AutoDrive/DriveKp", Constants.AUTO_DRIVE_P_CONTROLLER);
  private final TunableNumber autoDriveKi =
      new TunableNumber("AutoDrive/DriveKi", Constants.AUTO_DRIVE_I_CONTROLLER);
  private final TunableNumber autoDriveKd =
      new TunableNumber("AutoDrive/DriveKd", Constants.AUTO_DRIVE_D_CONTROLLER);
  private final TunableNumber autoTurnKp =
      new TunableNumber("AutoDrive/TurnKp", Constants.AUTO_TURN_P_CONTROLLER);
  private final TunableNumber autoTurnKi =
      new TunableNumber("AutoDrive/TurnKi", Constants.AUTO_TURN_I_CONTROLLER);
  private final TunableNumber autoTurnKd =
      new TunableNumber("AutoDrive/TurnKd", Constants.AUTO_TURN_D_CONTROLLER);
  ...
  private final double trackwidthMeters = Constants.TRACKWIDTH_METERS;
  private final double wheelbaseMeters = Constants.WHEELBASE_METERS;
  ...
  public Drivetrain(
    ...
    this.gyroIO = new Pigeon2(gyroID, Constants.CAN_BUS_NAME);
  ...
  private Rotation2d getRotation() {
    if (getGyroIsConnected()) {
      return Rotation2d.fromDegrees(getYaw() + this.gyroOffset);
    } else {
  ...
  public void setGyroOffset(double expectedYaw) {
    ...
    if (getGyroIsConnected()) {
      this.gyroOffset = expectedYaw - getYaw();
    } else {
      this.gyroOffset = 0;
  ...
  public boolean getGyroIsConnected() {
    ...
    if (gyroIO.getLastError().equals(ErrorCode.OK)) {
      isConnected = true;
      return true;
    }
    return false;
  ...
  public double getGyroVelocityDegPerSec() {
    gyroIO.getRawGyro(xyzDps);
    return xyzDps[2];
  }
